...
Logo components are named `MondayLogo`, `ClickUpLogo`, `PipedriveLogo`, `TilioLogo` (brand: **Tillio** — two l's), `OpenMercatoLogo`, `ClaudeLogo`, `N8nLogo`, `OpenCodeLogo`, `PythonLogo`. Static assets live in `public/assets/logos/`. To add a new logo: create a component here and add its `component` key to `HeroMetaLogo` in `site.ts`.
